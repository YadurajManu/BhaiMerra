...
export const PAGES = {
  ...
  docs: {
    ...
    blocks: [
      ...
      P('Fleet CLI is currently installed from this source repository. The hosted control plane is selected automatically; use --api or FLEET_API when you run your own control plane.'),
  ...
  'docs/fleet-yaml': {
    ...
    kicker: 'reference',
    lede: 'The declarative file that describes services. Validate and apply it from the CLI or dashboard; a connected GitHub repository can apply the exact pushed version automatically.',
    updated: '21 Aug 2026',
    blocks: [
      H('Top level'),
      { t: 'kv', rows: [
      ...
      { t: 'table', head: ['Field', 'Type', 'Default', 'Notes'], rows: [
        ...
        ['repo', 'repository URL', '—', 'Repository to fetch for GitHub push deploys.'],
        ['image', 'ref', '—', 'Pre-built image. Skips the build step.'],
        ['placement', 'enum', 'flexible', 'flexible | preferred | pinned'],
        ['node', 'node name', '—', 'Required when placement is pinned or preferred.'],
      ...
      { t: 'kv', rows: [
        ...
        ['manual', 'Stay where it landed until you explicitly reschedule it.'],
      ]},
  ...
  'docs/cli': {
    ...
    blocks: [
      H('Install'),
      P('Until the CLI is published as a package, build and link it from this repository. The default control plane is https://fleetapi.plastikworld.xyz; pass --api or set FLEET_API for a self-hosted installation.'),
      { t: 'code', lang: 'sh', lines: [
